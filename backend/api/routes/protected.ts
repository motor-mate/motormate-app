...
import { IVeicoliController, VeicoliController } from '../../controllers/VeicoliController';
...
router.get('/get/marche', authenticateJWT, async (req: Request, res: Response) => {
...
router.get('/get/modelli', authenticateJWT, async (req: Request, res: Response) => {
	...
	if (marca === '') {
		res.status(400).json({ message: 'Marca non specificata' });
		return;
	}
...
router.get('/get/userVehicles', authenticateJWT, async (req: Request, res: Response) => {
	...
	const controller: IVeicoliController = VeicoliController.instance;
	if (typeof id_utente === 'string') {
		const results = await controller.getVeicoli(id_utente);
		res.json(results);
	}
});
